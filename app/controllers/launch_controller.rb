class LaunchController < ActionController::Base
  ...
  def set_user(launch_params)
    if user_exists?
      ...
      @@user = User.create(full_name: launch_params["lis_person_name_full"],
    ...
    def request_access
      url = "https://#{@@domain}/login/oauth2/auth?client_id=37000000000000001&response_type=code&redirect_uri=http://localhost:3000/oauth2response&state=temp"
      redirect_to url
    end
